...
class MHCovid:
    ...
    def show(self, model, images, labels, r=4, c=4):
        ...
        for i in range(r):
            for j in range(c):
                ...
                plt.axis('off')
                if ((r*i+j) % 2):
                    index = covid_indexes[np.random.randint(
                        len(covid_indexes))]
                    output = round(
                        float(model(images[index:index+1])[0][0]), 3)
                    plt.imshow(images[index])
                    plt.title(
                        f'COVID-19: {self.label[int(labels[index])]} ,    Prediction: {output}')
                else:
                    index = uncovid_indexes[np.random.randint(
                        len(uncovid_indexes))]
                    output = round(
                        float(model(images[index:index+1])[0][0]), 3)
                    plt.imshow(images[index])
    ...
    def predict(self, model, imagefile, show=True):
        ...
        image = cv2.resize(cv2.cvtColor(cv2.imread(
            imagefile), cv2.COLOR_BGR2RGB), (224, 224))/255.
        output = round(
            float(model(np.array([image], dtype=np.float32))[0][0]), 3)
        plt.figure(figsize=(7, 7))
        plt.axis('off')
        plt.imshow(image)
        plt.title(
            f'Mohammad Hossein Amini (mhamini@aut.ac.ir)\n Predicted: {output}')
        return output
